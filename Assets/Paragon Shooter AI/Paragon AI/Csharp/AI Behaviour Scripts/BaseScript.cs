...
namespace ParagonAI
{
    public class BaseScript : MonoBehaviour
    {
        ...
        public void CheckToSeeIfWeShouldDodge()
        {
            ...
            if (shouldTryAndDodge && myTransform && targetTransform && !inCover && !isDodging && !isSprinting && !Physics.Linecast(myTransform.position, targetTransform.position, layerMask) && Vector3.Distance(myTransform.position, targetTransform.position) > minDistToDodge)
            {
                if (canOverrideBehaviour)
                {
